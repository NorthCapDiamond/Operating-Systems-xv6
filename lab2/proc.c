...
{  
  ...
  processes.pid = -1;
  processes.state = UNUSED;
  processes.next_proc = &processes;
  processes.prev_proc = &processes;
}
...
allocproc(void)
{
  ...
  p->prev_proc = &processes;
  p->next_proc = processes.next_proc;
  processes.next_proc->prev_proc = p;
  processes.next_proc = p;
...
reparent(struct proc *p)
{
  ...
  for(pp = processes.next_proc; pp != &processes; pp = pp->next_proc){
    if(pp->parent == p){
      pp->parent = initproc;
      propriate_wakeup(initproc);
    }
  }
}
...
sleep(void *chan, struct spinlock *lk)
{
  ...
  if(lk != &list_lock){
    release(&list_lock);  //DOC: sleeplock1
    acquire(lk);
  }
}
...
void propriate_wakeup(void *chan){
 struct proc *p;
